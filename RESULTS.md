...
| 5 agentic web build | **Three engines timed, ladder leader = mainline; re-roll done.** Dialed (64k, q4_0 KV, MTP n3): **mainline HIP 1131 s** (decode 49.7, prefill 279.1, accept 72.3%, peak 21.2 GiB) beats **buun-vk+MTP** (1282 s & 1468 s re-roll — same config, one-shot variance band ≈15%; decode 45.5; accept 72.1%) by 12-23% and **buun-vk control 1630 s** (31.6) by 31%. MTP ≈ +44% decode; acceptance identical across engines (fork MTP == mainline MTP); remaining fork value = TurboQuant KV only (q4_0 used in all ladder runs — turbo4 is HIP-only). Site quality = one-shot dice (buun attempt 1 FAILed, wiped; re-roll + mainline render balanced & grounded). **hipfire VOIDED** — no OpenAI tool_calls (arch gap, §6) |

...
| 5 agentic web build | **Three engines timed, ladder leader = mainline.** Dialed (64k, q4_0 KV, Q4_K_M): **mainline HIP+MTP 1131 s** (decode 49.7, prefill 279.1, accept 72.3%, peak 21.2 GiB) beats **buun-vk+MTP 1282 s** (45.5, 249.6, 72.1%, 23.1 GiB) by 12% and **buun-vk control 1630 s** (31.6, 195.5, 17.2 GiB) by 31%. MTP ≈ +44% decode, -21-31% wall — fork's MTP is equivalent (accept within 0.2 pt); remaining fork value = TurboQuant KV only. Site quality = one-shot dice (mainline site renders balanced+grounded; first buun MTP attempt FAILed and was wiped per owner — re-run queued). **hipfire VOIDED** — no OpenAI tool_calls (arch gap, §6) |
